 ...
 */
function removeLastOccurrences(a, b) {
  const index = a.lastIndexOf(b);
  return index !== -1 ? a.slice(0, index) + a.slice(index + b.length) : a;
}
...
function sumOfCodes(a) {
  ...
  const b = a.split('');
  return b.reduce((arr, el) => arr + el.charCodeAt(), 0);
}
 ...
 */
function formatTime(tim, sec) {
  return `${String(tim).padStart(2, '0')}:${String(sec).padStart(2, '0')}`;
}
...
function isPalindrome(a) {
  const c = a.replace(/[^\w]/g, '').toLowerCase();
  const b = [...c].reverse().join('');
  return c === b;
}
...
function encodeToRot13(str) {
  let newStr = '';
  [...str].forEach((el) => {
    let numStr = el.charCodeAt(0);
    if (numStr >= 65 && numStr <= 122) {
      if (numStr < 97) {
        numStr = el.toLowerCase().charCodeAt(0) + 13;
        if (numStr > 122) {
          numStr = -(123 - numStr) + 97;
        }
        newStr += String.fromCharCode(numStr).toUpperCase();
      } else if (numStr + 13 > 122) {
        numStr = -(123 - (numStr + 13)) + 97;
        newStr += String.fromCharCode(numStr);
      } else {
        newStr += String.fromCharCode(numStr + 13);
      }
    } else {
      newStr += el;
    }
